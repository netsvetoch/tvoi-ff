# Theming — `--g-*` tokens, `ThemeProvider`, branding, custom themes

> **Source of truth:** this is a verbatim copy of
> [`@gravity-ui/uikit`'s `docs/theming.md`](https://github.com/gravity-ui/uikit/blob/main/docs/theming.md).
> The current version in the installed `node_modules` may differ — for the exact
> file matching your version, read
> `node_modules/@gravity-ui/uikit/build/docs/guides/theming.md` (or fetch the
> version-pinned copy at `gravity-ui.com/llms/uikit/<major>/llms.txt`).
>
> This copy is bundled offline. If a token you need is missing or looks different
> here, prefer the version-matched source over this snapshot.

For the **traps** — what goes wrong when theming a Gravity UI app (partial
overrides, brand-on-brand contrast, scoped vs nested `ThemeProvider`, the
customization ladder) — see [`../rules/hallucination-traps.md`](../rules/hallucination-traps.md).
That file complements this reference rather than duplicating it.
